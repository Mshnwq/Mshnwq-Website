<script lang="ts">
  ...
  import { rssFeedUrls, extraFeeds } from '$lib/store/BlogStore';
  import clickOutside from '$lib/directives/clickOutside';
  import type { RssUrlItem } from '$lib/types/RssXml';
  ...
  const toggleFeed = (feed: RssUrlItem): void => {
    const isFound = get(rssFeedUrls).find((cFeed: { url: string; }) => cFeed.url === feed.url);
    if (isFound) {
      // Remove specified feed from store
      rssFeedUrls.set(
        get(rssFeedUrls).filter((cFeed: { url: string; }) => cFeed.url !== feed.url)
      );
    } else {
